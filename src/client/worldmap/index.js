...
WorldMap.prototype = {
  ...
  initControls: function () {
    ...
    this.controlsTrackball = new THREE.TrackballControls(
      this.camera,
    ...
    this.controlsPinchZoom = new THREE.PinchZoomControls(
      this.camera,
      this.renderer.domElement
    );
  ...
  createCountries: function () {
    ...
    window.setTimeout(function () {
      ...
      worldMap.tweenScale = new TWEEN.Tween(worldMap.countriesObject3D.scale)
        .to(
          { x: scaleFinal, y: scaleFinal, z: scaleFinal },
          Config.introRotateDuration
        ) // 3500
        .delay(0)
        .onStart(function () {
          worldMap.animationProps.interpolatePos = 1.0;
      ...
      worldMap.tweenRotation = new TWEEN.Tween(
        worldMap.countriesObject3D.rotation
      )
  ...
  updateCountryColorsOneByOne: function () {
    ...
    new TWEEN.Tween({})
      ...
      .onUpdate(function (time) {
        ...
        worldMap.animationProps.colorChangeID = parseInt(
          time * worldMap.countries.length
        );

        worldMap.updateCountryColors(
          idLast,
          worldMap.animationProps.colorChangeID
        );
  ...
  updateCountryColors: function (start, end, pos) {
    ...
    for (var i = start; i < end; i++) {
      ...
      if (country.listItem)
        country.listItem
          .find(".box")
          .css("background-color", "#" + country.color.getHexString());
  ...
  setParamsFromSearch: function () {
    ...
    if (source !== undefined && source !== "") {
      for (var i = 0; i < this.countries.length; i++) {
        ...
        if (
          cleanURLString(name) === cleanURLString(source.toLowerCase()) &&
          CountryDataHelpers.isCountry(country)
        ) {
    ...
    if (destination !== undefined && destination !== "") {
      for (i = 0; i < this.countries.length; i++) {
        ...
        if (
          cleanURLString(name) === cleanURLString(destination.toLowerCase()) &&
          CountryDataHelpers.isCountry(country)
        ) {
  ...
  selectCountryFromMap: function (event) {
    ...
    if (intersects.length > 0) {
      if (
        intersects[0].object.countryObject &&
        this.selectedCountry !== intersects[0].object.countryObject
      ) {
          ...
          if (this.mode === "destinations") {
            if (intersects[0].object.countryObject.disputed) {
              if (
                ...
                this.setSelectedDisputedCountry(
                  intersects[0].object.countryObject
                );
              }
            } else {
              if (
                ...
                if (this.selectedCountry && this.selectedCountry.disputed) {
                  ...
                  this.setSelectedDestinationCountry(
                    intersects[0].object.countryObject
                  );
                  this.trackEvent(
                    "destinationCountryMapClick",
                    intersects[0].object.countryObject.name
                  );
                }
              } else {
                this.setSelectedCountry(intersects[0].object.countryObject);
                this.trackEvent(
                  "sourceCountryMapClick",
                  intersects[0].object.countryObject.name
                );
              }
            }
            ...
            if (intersects[0].object.countryObject.disputed) {
              if (
                ...
                this.setSelectedDisputedDestinationCountry(
                  intersects[0].object.countryObject
                );
              }
            } else {
      ...
      if (this.mode === "destinations") {
        if (
          ...
          event.altKey ||
          event.metaKey ||
          (destinationFocus && $("#destination-country-dropdown").val() !== "")
        ) {
  ...
  clearBothSelectedCountries: function () {
    ...
    if (this.selectedCountry || this.selectedDestinationCountry) {
      ...
      if (this.selectedCountry) {
        if (this.selectedCountry.listItem !== undefined)
  ...
  setSelectedDestinationCountry: function (selectedDestinationCountry) {
    ...
    if (!this.introRunning) {
      ...
      if (this.selectedDestinationCountry) {
        ...
        UI.setDestinationCountryDropdownValue(
  ...
  updateCountrySelection: function () {
    ...
    if (this.mode === "destinations") {
      if (this.selectedCountry && this.selectedDestinationCountry) {
        ...
        if (destinations.length > 0) {
          ...
          for (d = 0; d < destinations.length; d++) {
            if (
              ...
              this.selectedDestinationCountry.visa_required =
                destinations[d].visa_required;
              this.selectedDestinationCountry.visa_title =
                destinations[d].visa_title;
          ...
          mainCountry = CountryDataHelpers.getCountryByName(
            this.countries,
            this.selectedCountry.sovereignt
          );
          if (mainCountry && mainCountry.visa_required !== "") {
            mainCountry.visa_required = "no";
            mainCountry.notes =
              "National of same sovereignty (exceptions may exist)";
            this.visaInformationFound = true;
        ...
        if (!this.visaInformationFound) {
          UI.setHeadline(
            "Data not available for nationals from " +
              CountryDataHelpers.getCountryNameWithArticle(
                this.selectedCountry
              ) +
              " travelling to " +
        ...
        if (destinations.length > 0) {
            ...
            for (var c = 0; c < this.countries.length; c++) {
              ...
              if (
                ...
                if (
                  ...
                  this.selectedCountry.populationReachable +=
                    this.countries[c].population;
                }
          ...
          UI.setHeadline(
            "Nationals from " +
              ...
              sovereignty +
              " can travel to <b>" +
              this.selectedCountry.numDestinationsFreeOrOnArrival +
      ...
      if (this.selectedCountry && this.selectedDestinationCountry) {
          ...
          for (d = 0; d < destinations.length; d++) {
            if (
              ...
              if (
                ...
                this.selectedDestinationCountry.populationAccepted +=
                  this.countries[i].population;
              }
            }
          }
        }
        ...
        var countries = CountryDataHelpers.getAllCountriesWithSameSovereignty(
          this.countries,
          this.selectedDestinationCountry.sovereignt
        );
      ...
      if (this.selectedCountry) {
        if (this.selectedCountry.gdp > 100) {
          value = Math.round(
            (this.selectedCountry.gdp / this.selectedCountry.population) *
              1000000
          );
          html +=
            "GDP per capita of " +
      ...
      if (this.selectedDestinationCountry) {
        if (this.selectedDestinationCountry.gdp > 100) {
          value = Math.round(
            (this.selectedDestinationCountry.gdp /
              this.selectedDestinationCountry.population) *
              1000000
          );
          html +=
            "GDP per capita of " +
...
function init() {
  ...
  log(
    "Loading Visa requirements from '" + Config.visaRequirementsFile + "' ..."
  );

  $.when(
    $.getJSON(Config.visaRequirementsFile).fail(function (error) {
      console.error("Error loading visa requirements", error);
      UI.updateLoadingInfo(
        '<span class="error">Error loading visa requirements.<br/>Please try again later.</span>'
      );
    })
  ).then(function (dataRequirements) {
    log(
      "Visa requirements loaded for " +
        dataRequirements.countries.length +
        " sovereignties"
    );
        ...
        $.when($.getJSON(Config.mapDataFile2)).then(function (dataCountries2) {
          ...
          for (var i = 0; i < dataCountries2.features.length; i++) {
            ...
            if (!found) {
              worldMap.dataCountries.features.push(feature2);
              log(
                "Adding country from higher res file: " +
                  feature2.properties.NAME +
                  ", type: " +
                  feature2.properties.TYPE
              );
            }
          }
